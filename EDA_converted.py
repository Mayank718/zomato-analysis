...
if set(["rate","votes"]).issubset(data.columns):
    ...
    display(top_success[["name","primary_cuisine","location_short","rate","votes","approx_cost_for_two_people","success_score"]].head(30))
...
if len(cluster_cols) >= 2:
    ...
    display(pd.DataFrame(km.cluster_centers_, columns=cluster_cols))
...
if "location_short" in data.columns and "rate" in data.columns:
    ...
    if not risky.empty:
        ...
        display(risky.reset_index().head(10))
    else:
...
if "primary_cuisine" in data.columns and set(["rate","approx_cost_for_two_people"]).issubset(data.columns):
    ...
    if not good.empty:
        ...
        display(good.sort_values(["avg_rate","count"], ascending=[False,False]).head(10))
    else:
